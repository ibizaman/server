...
class StatusService {
	...
	public function revertMultipleUserStatus(array $userIds, string $messageId, string $status): void {
		...
		$userStatuses = $this->mapper->findByUserIds($userIds);
		...
		// For users with both delete the status when matching
		foreach ($statuesToDelete as $userId => $statusId) {
			if (isset($backups[(string) $userId])) {
				$restoreIds[] = $backups[(string) $userId];
			}
		}
